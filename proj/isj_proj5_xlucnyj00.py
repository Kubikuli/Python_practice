...
def strs_for_number(number: int, digit2chars: dict[int,str]) -> list[str]:
    ...
    str_components_to_product = [digit2chars[int(num)] for num in str(number)] # your code 
...
def words_with_same_number(words: list[str],
    ...
    char2digit_as_str = {char:key for key,vals in digit2chars.items() for char in vals} # your code 
    ...
    for w in words:
        ...
        represen = ""
        for c in w:
            represen += (str(char2digit_as_str[c]))
        numb2words[represen].append(w)
    ...
    # associated list 
    return {int(key): val for key,val in numb2words.items() if len(val) > 1} # your code
